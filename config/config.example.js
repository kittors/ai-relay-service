...
const config = {
  ...
  redis: {
    ...
    // 某些受限环境禁用了 INFO 等命令，关闭 ready check 可绕过
    enableReadyCheck: process.env.REDIS_ENABLE_READY_CHECK !== 'false',
    // TLS 细化配置（当 enableTLS=true 时生效）
    tlsOptions: {
      rejectUnauthorized:
        process.env.REDIS_TLS_REJECT_UNAUTHORIZED === 'false' ? false : true,
  ...
  // 💳 支付配置
  payment: {
    // 购买后跳转的支付地址（外部收银台或自建支付页），示例：https://pay.example.com/checkout
    ...
    xunhu: {
      ...
      backupQueryUrl:
        process.env.XH_QUERY_URL_BACKUP || 'https://api.dpweixin.com/payment/query.html',
      plugin: process.env.XH_PLUGIN || '',
      version: process.env.XH_VERSION || '1.1',
      type: process.env.XH_TYPE || '', // WAP/JSAPI 等
      wapUrl: process.env.XH_WAP_URL || '',
      wapName: process.env.XH_WAP_NAME || '',
      // 通知与返回URL（可选，未设置时使用自动拼接的站点URL）
