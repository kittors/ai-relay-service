...
export const useApiStatsStore = defineStore('apistats', () => {
  ...
  const oemSettings = ref({
    ...
    siteIcon: '',
    siteIconData: '',
    showAdminButton: false, // 默认为隐藏，避免首屏闪现
    ldapEnabled: false
  })
  // 产品套餐（公开页面使用）
  ...
  async function loadProductSettings() {
    try {
      const result = await apiStatsClient.getProductSettings()
      if (result && result.success && result.data && Array.isArray(result.data.plans)) {
        productPlans.value = result.data.plans
      } else {
        productPlans.value = []
      }
    } catch (err) {
