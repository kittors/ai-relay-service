...
export const useDashboardStore = defineStore('dashboard', () => {
  ...
  const groupLimitsSummary = ref({
    platforms: {
      claude: { updatedAt: null, overall: null, groups: [] },
      openai: { updatedAt: null, overall: null, groups: [] }
    }
  })
  ...
  async function loadDashboardData(timeRange = null) {
    ...
    try {
      ...
      const [dashboardResponse, todayCostsResponse, totalCostsResponse, limitsSummaryResp] =
        await Promise.all([
          apiClient.get('/admin/dashboard'),
          apiClient.get(`/admin/usage-costs?period=${costsParams.today}`),
          apiClient.get(`/admin/usage-costs?period=${costsParams.all}`),
          apiClient.get('/admin/account-limits/summary')
        ])
      ...
      // 更新分组窗口/限额汇总
      if (limitsSummaryResp && limitsSummaryResp.success) {
        groupLimitsSummary.value =
          limitsSummaryResp.data ||
          {
            platforms: {
              claude: { updatedAt: null, overall: null, groups: [] },
              openai: { updatedAt: null, overall: null, groups: [] }
  ...
  // 单独加载分组窗口/限额汇总，可选刷新外部快照
  async function loadGroupLimitsSummary(refresh = false) {
    try {
      const url = refresh
        ? '/admin/account-limits/summary?refresh=true'
        : '/admin/account-limits/summary'
