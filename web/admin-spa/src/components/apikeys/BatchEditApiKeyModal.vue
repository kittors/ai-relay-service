...
// 刷新账号列表（如果父组件传入 accounts，则优先复用，避免重复请求）
const refreshAccounts = async () => {
  accountsLoading.value = true
  try {
    if (props.accounts && (props.accounts.claude?.length || props.accounts.openai?.length || props.accounts.gemini?.length || props.accounts.bedrock?.length || props.accounts.droid?.length)) {
      // 直接复用父级传入的数据
      const openaiAccounts = []
      ;(props.accounts.openai || []).forEach((account) =>
        openaiAccounts.push({ ...account, platform: 'openai', isDedicated: account.accountType === 'dedicated' })
      )
      localAccounts.value = {
        claude: (props.accounts.claude || []).map((a) => ({ ...a, isDedicated: a.accountType === 'dedicated' })),
      ...
      if (resp.data?.openaiAccounts) {
        localAccounts.value.openai = (resp.data.openaiAccounts || []).map((account) => ({
          ...account,
          isDedicated: account.accountType === 'dedicated'
        }))
      }
