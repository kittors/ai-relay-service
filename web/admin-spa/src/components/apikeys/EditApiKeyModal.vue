      ...
      >
        ...
        >
          ...
          <div>
            ...
            <div class="space-y-2">
              <input
                v-model="form.dailyRequestsLimit"
              ...
              />
              <p class="text-xs text-gray-500 dark:text-gray-400">
                设置此 API Key 每日请求次数上限，超过限制将返回 429；0 表示不限制
              </p>
            </div>
          </div>
...
// 标签相关
const newTag = ref('')
const localAvailableTags = ref([])

// 计算未选择的标签
const unselectedTags = computed(() => {
  return localAvailableTags.value.filter((tag) => !form.tags.includes(tag))
})
    ...
    const data = {
      ...
      weeklyOpusCostLimit:
        form.weeklyOpusCostLimit !== '' && form.weeklyOpusCostLimit !== null
          ...
          : 0,
      dailyRequestsLimit:
        form.dailyRequestsLimit !== '' && form.dailyRequestsLimit !== null
          ? parseInt(form.dailyRequestsLimit)
          : 0,
...
// 刷新账号列表（支持按平台精简拉取）
const refreshAccounts = async (platformOverride) => {
  accountsLoading.value = true
  try {
    const platform = platformOverride || form.permissions || 'all'
    ...
    if (resp.data?.claudeAccounts) {
      resp.data.claudeAccounts.forEach((account) => {
        claudeAccounts.push({
          ...account,
    ...
    if (resp.data?.claudeConsoleAccounts) {
      resp.data.claudeConsoleAccounts.forEach((account) => {
        claudeAccounts.push({
          ...account,
    ...
    if (resp.data?.geminiAccounts) {
      localAccounts.value.gemini = (resp.data.geminiAccounts || []).map((account) => ({
        ...account,
    ...
    if (resp.data?.openaiAccounts) {
      ;(resp.data.openaiAccounts || []).forEach((account) => {
        openaiAccounts.push({
          ...account,
          platform: 'openai',
          isDedicated: account.accountType === 'dedicated'
    ...
    if (resp.data?.openaiResponsesAccounts) {
      ;(resp.data.openaiResponsesAccounts || []).forEach((account) => {
        openaiAccounts.push({
          ...account,
    ...
    if (resp.data?.bedrockAccounts) {
      localAccounts.value.bedrock = (resp.data.bedrockAccounts || []).map((account) => ({
        ...account,
        isDedicated: account.accountType === 'dedicated'
      }))
    }

    if (resp.data?.droidAccounts) {
      localAccounts.value.droid = (resp.data.droidAccounts || []).map((account) => ({
        ...account,
        platform: 'droid',
        isDedicated: account.accountType === 'dedicated'
      }))
    ...
    // 处理分组数据
    if (resp.data?.accountGroups) {
      const allGroups = resp.data.accountGroups || []
...
onMounted(async () => {
  try {
    // 并行加载轻量上下文：客户端、用户、Key详情（标签来自页面传入）
    const [clients, , keyResp] = await Promise.all([
      clientsStore.loadSupportedClients(),
      loadUsers(),
      apiClient.get(`/admin/api-keys/${props.apiKey.id}`).catch(() => ({ success: false }))
    ])

    supportedClients.value = clients || []
    localAvailableTags.value = Array.isArray(props.availableTags) ? props.availableTags : []
    // 用详情接口数据覆盖回显（若可用），避免依赖列表数据的延迟
    if (keyResp && keyResp.success && keyResp.data) {
      const d = keyResp.data
      form.name = d.name
      form.tokenLimit = d.tokenLimit || ''
      form.rateLimitCost = d.rateLimitCost || ''
      form.rateLimitWindow = d.rateLimitWindow || ''
      form.rateLimitRequests = d.rateLimitRequests || ''
      form.concurrencyLimit = d.concurrencyLimit || ''
      form.dailyCostLimit = d.dailyCostLimit || ''
      form.totalCostLimit = d.totalCostLimit || ''
      form.weeklyOpusCostLimit = d.weeklyOpusCostLimit || ''
      form.dailyRequestsLimit = d.dailyRequestsLimit || ''
      form.permissions = d.permissions || 'all'
      if (d.claudeConsoleAccountId) {
        form.claudeAccountId = `console:${d.claudeConsoleAccountId}`
      } else {
        form.claudeAccountId = d.claudeAccountId || ''
      }
      form.geminiAccountId = d.geminiAccountId || ''
      form.openaiAccountId = d.openaiAccountId || ''
      form.bedrockAccountId = d.bedrockAccountId || ''
      form.droidAccountId = d.droidAccountId || ''
      form.restrictedModels = Array.isArray(d.restrictedModels) ? d.restrictedModels : []
      form.allowedClients = Array.isArray(d.allowedClients) ? d.allowedClients : []
      form.tags = Array.isArray(d.tags) ? d.tags : []
      form.enableModelRestriction = !!d.enableModelRestriction
      form.enableClientRestriction = !!d.enableClientRestriction
      form.isActive = d.isActive !== undefined ? !!d.isActive : true
      form.ownerId = d.userId || 'admin'
    } else {
      // 回退：使用传入的 props.apiKey 回显
      ...
      form.dailyRequestsLimit = props.apiKey.dailyRequestsLimit || ''
    ...
    localAvailableTags.value = []
  }
  // 复用父级传入的账号与分组（精简视图足够用于选择器展示）
  try {
    if (props.accounts) {
      const openaiAccounts = []
      ;(props.accounts.openai || []).forEach((a) =>
        openaiAccounts.push({ ...a, platform: 'openai', isDedicated: a.accountType === 'dedicated' })
      )
      ;(props.accounts.openaiResponses || []).forEach((a) =>
        openaiAccounts.push({
          ...a,
          platform: 'openai-responses',
      ...
      localAccounts.value = {
        claude: (props.accounts.claude || []).map((a) => ({ ...a, platform: 'claude-oauth', isDedicated: a.accountType === 'dedicated' })),
        gemini: (props.accounts.gemini || []).map((a) => ({ ...a, isDedicated: a.accountType === 'dedicated' })),
        openai: openaiAccounts,
        bedrock: (props.accounts.bedrock || []).map((a) => ({ ...a, isDedicated: a.accountType === 'dedicated' })),
        droid: (props.accounts.droid || []).map((a) => ({ ...a, platform: 'droid', isDedicated: a.accountType === 'dedicated' })),
