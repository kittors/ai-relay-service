...
const currentSiteUrl = computed(() => {
  const custom = (oemSettings.value.tutorialUrl || '').trim()
...
// 构建复制文案
const buildCopyText = (apiKey) => {
  const template = (oemSettings.value.copyInfoTemplate || '').trim()
  const siteUrl = currentSiteUrl.value
  if (template) {
    // 占位符替换：{siteUrl}, {apiKey}
    return template.replace(/\{siteUrl\}/g, siteUrl).replace(/\{apiKey\}/g, apiKey)
  }
  // 默认文案
  return `您可以通过访问 "${siteUrl}" 查看使用教程和查看您的使用情况。\n您的API Key是：${apiKey}`
}
...
const copyApiKey = async () => {
  ...
  // 构建复制内容
  const configText = buildCopyText(key)
