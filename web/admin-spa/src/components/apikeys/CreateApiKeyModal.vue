...
onMounted(async () => {
  ...
  // 若父级未传入或为空，再发起一次精简请求
  const totalProvided =
    (localAccounts.value?.claude?.length || 0) +
    (localAccounts.value?.gemini?.length || 0) +
    (localAccounts.value?.openai?.length || 0) +
    (localAccounts.value?.bedrock?.length || 0) +
    (localAccounts.value?.droid?.length || 0)
  if (!totalProvided) {
