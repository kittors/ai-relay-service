      ...
      <div v-if="!multiKeyMode" class="space-y-4 md:space-y-5">
        ...
        <div class="mt-2">
          <div class="mb-2 flex items-center justify-between">
            ...
            >
            <span class="text-xs text-gray-500 dark:text-gray-400 md:text-sm">
              <template v-if="statsData.limits.dailyRequestsLimit > 0">
                {{ formatNumber(statsData.limits.currentDailyRequests) }} /
                {{ formatNumber(statsData.limits.dailyRequestsLimit) }} 次
              </template>
              <template v-else>
                <i class="fas fa-infinity text-gray-400" /> 不限制
              </template>
            </span>
          </div>
          <div
            v-if="statsData.limits.dailyRequestsLimit > 0"
            class="h-2 w-full rounded-full bg-gray-200 dark:bg-gray-700"
          >
            <div
              class="h-2 rounded-full transition-all duration-300"
              :class="getDailyRequestsProgressColor()"
              :style="{ width: getDailyRequestsProgress() + '%' }"
            />
          </div>
          <div v-else class="h-2 w-full rounded-full bg-gray-200">
            <div class="h-2 rounded-full bg-green-500" style="width: 0%" />
...
// 每日请求进度
const getDailyRequestsProgress = () => {
  const limit = statsData.value.limits.dailyRequestsLimit || 0
  if (!limit) return 0
  const used = statsData.value.limits.currentDailyRequests || 0
  return Math.min((used / limit) * 100, 100)
}

const getDailyRequestsProgressColor = () => {
  const p = getDailyRequestsProgress()
