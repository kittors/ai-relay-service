<template>
  <div class="theme-toggle-container">
    ...
    <!-- 下拉菜单模式 - 极简三段开关（点击轮换：浅色/自动/深色） -->
    <div v-else-if="mode === 'dropdown'" class="theme-switch-wrapper">
      <button
        aria-label="切换主题"
        class="theme-switch-min"
        :class="themeStore.themeMode"
        :title="themeTooltip"
        @click="handleCycleTheme"
      >
        <i class="fas fa-sun" />
...
.theme-switch-min:hover {
  filter: brightness(0.98);
}
.theme-switch-min:active {
  transform: scale(0.98);
}

:global(html.dark) .theme-switch-min {
...
.theme-switch-min.light i.fa-sun,
.theme-switch-min.auto i.fa-circle-half-stroke,
.theme-switch-min.dark i.fa-moon {
  color: #2563eb; /* blue-600 */
}

.theme-switch-min .toggle-thumb {
  position: absolute;
  top: 3px;
  left: 4px;
  width: 22px;
...
:global(html.dark) .theme-switch-min .toggle-thumb {
...
/* 位置：左(浅色)/中(自动)/右(深色) */
.theme-switch-min.light .toggle-thumb {
  transform: translateX(0);
}
.theme-switch-min.auto .toggle-thumb {
  transform: translateX(21px);
}
.theme-switch-min.dark .toggle-thumb {
  transform: translateX(42px);
}

/* 旧的创意切换开关样式（保留以防回滚，但不再使用模板） */
/* 创意切换开关样式 */
.theme-switch-wrapper {
  @apply inline-flex items-center;
}
