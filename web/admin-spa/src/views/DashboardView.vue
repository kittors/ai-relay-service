<template>
  <div>
    <!-- 分组窗口/限额统计 Banner -->
    <GroupLimitsBanner :limits="groupLimitsSummary" :refreshing="groupLimitsRefreshing" @refresh="handleGroupLimitsRefresh" />
...
import GroupLimitsBanner from '@/components/dashboard/GroupLimitsBanner.vue'
...
const groupLimitsRefreshing = ref(false)

async function handleGroupLimitsRefresh() {
  if (groupLimitsRefreshing.value) return
  groupLimitsRefreshing.value = true
  try {
    await loadGroupLimitsSummary(true)
  } finally {
    groupLimitsRefreshing.value = false
  }
}
