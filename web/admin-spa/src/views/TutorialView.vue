    ...
    <div v-if="activeTutorialSystem === 'windows'" class="tutorial-content">
      ...
      <div class="mb-4 sm:mb-10 sm:mb-6">
        ...
        >
          ...
          <p class="mb-3 text-sm text-gray-700 dark:text-gray-300 sm:mb-4 sm:text-base">
            先下载安装脚本，然后在 PowerShell 中执行（建议以管理员身份运行 PowerShell）：
          </p>
          <div class="mb-3 flex flex-wrap gap-2">
            <a
              class="inline-flex items-center rounded-md bg-green-600 px-3 py-2 text-xs font-semibold text-white hover:bg-green-700 sm:text-sm"
              download
              href="https://site.07230805.xyz/autoInstallCodex.ps1"
              @click.prevent="
                downloadScript(
                  'https://site.07230805.xyz/autoInstallCodex.ps1',
                  'autoInstallCodex.ps1'
                )
              "
            >
              <i class="fas fa-download mr-2" />下载 Windows 脚本（autoInstallCodex.ps1）
            </a>
          </div>
          <div
            class="mb-4 overflow-x-auto rounded-lg bg-gray-900 p-3 font-mono text-xs text-green-400 sm:p-4 sm:text-sm"
          >
            <div class="mb-2"># 在 PowerShell 中（必要时先放宽执行策略，仅限当前会话）</div>
            <div class="whitespace-nowrap text-gray-300">
              Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force
            </div>
            <div class="mb-2 mt-3"># 进入脚本所在目录并执行</div>
            <div class="whitespace-nowrap text-gray-300">cd "你的下载目录"</div>
            <div class="whitespace-nowrap text-gray-300">./autoInstallCodex.ps1</div>
          </div>
          <p class="text-sm text-gray-600 dark:text-gray-400">
            脚本会自动：卸载旧版本、重新安装
            <code class="bg-gray-100 px-1 dark:bg-gray-800">@openai/codex</code>、创建
            <code class="bg-gray-100 px-1 dark:bg-gray-800">~/.codex/config.toml</code> 与
            <code class="bg-gray-100 px-1 dark:bg-gray-800">~/.codex/auth.json</code>，并提示你输入
            API Key。
          </p>
          ...
          >
            ...
            <div
              class="overflow-x-auto rounded bg-gray-900 p-2 font-mono text-xs text-green-400 sm:p-3 sm:text-sm"
            >
              <div class="text-gray-300">--- 正在开始 Codex 配置脚本 ---</div>
              <div class="text-gray-300">1. 卸载全局 @openai/codex 包...</div>
              <div class="text-gray-300">2. 删除 ~/.codex 目录...</div>
              <div class="text-gray-300">3. 全局安装 @openai/codex 包...</div>
              <div class="text-gray-300">4. 创建配置目录和 config.toml...</div>
              <div class="text-gray-300">--- Codex 配置完成，现在需要您的 API 密钥 ---</div>
              <div class="text-gray-300">
                请输入您的 OpenAI API 密钥 (cr-xxxxxxxxxxxxxxxxxxxx): ********
              </div>
              <div class="text-gray-300">--- Codex 配置脚本执行完毕！ ---</div>
            </div>
            <ul class="mt-2 space-y-1 text-xs text-blue-700 dark:text-blue-300 sm:text-sm">
              <li>• 建议使用 PowerShell 而不是 CMD，功能更强大</li>
              <li>• 如遇执行策略限制，可先运行上方放宽命令，或以管理员身份运行 PowerShell</li>
            </ul>
          </div>
        </div>
        ...
        <div
          class="rounded-lg border border-green-200 bg-green-50 p-3 dark:border-green-500/40 dark:bg-green-950/30 sm:p-4"
        >
          <h6 class="mb-2 font-medium text-green-800 dark:text-green-300">验证 CodeX 安装</h6>
          ...
          <div
            class="overflow-x-auto rounded bg-gray-900 p-2 font-mono text-xs text-green-400 sm:p-3 sm:text-sm"
          >
            <div class="whitespace-nowrap text-gray-300">codex -V</div>
            <div class="whitespace-nowrap text-gray-300">codex-cli 0.46.0</div>
          </div>
          <p class="mt-2 text-sm text-green-700 dark:text-green-300">
            如果出现以上两行输出，恭喜你！CodeX CLI 已经成功安装。
          </p>
        </div>
      </div>
      ...
      <div class="mb-6 sm:mb-10">
        ...
        >
          ...
          <p class="mb-3 text-sm text-gray-700 dark:text-gray-300 sm:mb-4 sm:text-base">
            使用上面的自动安装脚本后，CodeX 已自动完成配置，通常无需额外设置环境变量。
            本节仅供高级用户参考或手动配置场景使用。
          </p>
        ...
        <div class="mt-8">
          ...
          <p class="mb-3 text-sm text-gray-700 dark:text-gray-300 sm:mb-4 sm:text-base">
            若你已运行上面的自动安装脚本，
            <code class="bg-gray-100 px-1 dark:bg-gray-800">~/.codex/config.toml</code>
            与
            <code class="bg-gray-100 px-1 dark:bg-gray-800">~/.codex/auth.json</code>
            已自动创建，通常无需再手动设置。本节为可选的高级配置说明。
          </p>
        ...
        >
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <ul
                class="mb-2 list-inside list-decimal space-y-1 text-sm text-indigo-700 dark:text-indigo-300"
              >
                <li>打开 VSCode，进入“扩展”</li>
                <li>搜索并安装插件：<strong>Codex – OpenAI’s coding agent</strong></li>
                <li>
                  确保命令行可运行
                  <code class="bg-indigo-100 px-1 dark:bg-indigo-900">codex -V</code>
                </li>
                <li>重启 VSCode 或重新加载窗口</li>
              </ul>
              <p class="text-xs text-indigo-600 dark:text-indigo-400">
                无需额外配置，插件会自动调用全局的 CodeX CLI，并使用
                <code class="bg-indigo-100 px-1 dark:bg-indigo-900">~/.codex/config.toml</code>
                和
                <code class="bg-indigo-100 px-1 dark:bg-indigo-900">~/.codex/auth.json</code>
                。
              </p>
            </div>
          </div>
    ...
    <div v-else-if="activeTutorialSystem === 'macos'" class="tutorial-content">
      ...
      <div class="mb-6 sm:mb-10">
        ...
        >
          ...
          <div class="mb-3 flex flex-wrap gap-2">
            <a
              class="inline-flex items-center rounded-md bg-purple-600 px-3 py-2 text-xs font-semibold text-white hover:bg-purple-700 sm:text-sm"
              download
              href="https://site.07230805.xyz/autoInstallCodex.sh"
              @click.prevent="
                downloadScript(
                  'https://site.07230805.xyz/autoInstallCodex.sh',
                  'autoInstallCodex.sh'
                )
              "
            >
              <i class="fas fa-download mr-2" />下载 macOS/Linux 脚本（autoInstallCodex.sh）
            </a>
          </div>
          <p class="mb-2 text-sm text-gray-700 dark:text-gray-300">赋予可执行权限并运行：</p>
          <div
            class="mb-3 overflow-x-auto rounded-lg bg-gray-900 p-3 font-mono text-xs text-green-400 sm:p-4 sm:text-sm"
          >
            <div class="whitespace-nowrap text-gray-300">chmod +x ./autoInstallCodex.sh</div>
            <div class="whitespace-nowrap text-gray-300">./autoInstallCodex.sh</div>
            <div class="mt-2 whitespace-nowrap text-gray-300"># 或使用 bash 直接运行</div>
            <div class="whitespace-nowrap text-gray-300">bash ./autoInstallCodex.sh</div>
          </div>
          <div
            class="rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-500/40 dark:bg-blue-950/30 sm:p-4"
          >
            ...
            >
              ...
              <div class="text-gray-300">1. 卸载全局 @openai/codex 包...</div>
              <div class="text-gray-300">2. 删除 ~/.codex 目录...</div>
              <div class="text-gray-300">3. 重新安装全局 @openai/codex 包...</div>
              <div class="text-gray-300">4. 创建 ~/.codex/config.toml</div>
              <div class="text-gray-300">--- Codex 配置完成，现在需要您的 API 密钥 ---</div>
              <div class="text-gray-300">
                请输入您的 OpenAI API 密钥 (cr-xxxxxxxxxxxxxxxxxxxx): ********
              </div>
            ...
            <p class="mt-2 text-xs text-blue-700 dark:text-blue-300">
              • 如果提示无权限，请在命令前加上
              <code class="bg-blue-100 px-1 dark:bg-blue-900">sudo</code>
            </p>
          </div>
        </div>
...
const downloadScript = async (url, filename) => {
  try {
    ...
    // 若 CORS 不允许或抓取失败：打开新窗口，用户可右键“另存为”
