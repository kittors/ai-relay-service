...
# 默认配置
DEFAULT_INSTALL_DIR="$HOME/ai-relay-service"
...
# 安装服务
install_service() {
    print_info "开始安装 AI Relay Service..."
    ...
    if ! git clone https://github.com/Wei-Shaw/ai-relay-service.git "$APP_DIR"; then
        print_error "克隆项目失败"
        return 1
    fi
...
EOF
    ...
    if git ls-remote --heads origin web-dist | grep -q web-dist; then
        ...
        git clone --depth 1 --branch web-dist --single-branch \
            https://github.com/Wei-Shaw/ai-relay-service.git \
...
update_service() {
    ...
    print_info "更新 AI Relay Service..."
...
uninstall_service() {
    ...
    print_warning "即将卸载 AI Relay Service"
    echo -n "确定要卸载吗？(y/N): "
    read -n 1 confirm
    echo
    ...
    if [[ "$backup" =~ ^[Yy]$ ]]; then
        local backup_dir="$HOME/ai-relay-backup-$(date +%Y%m%d%H%M%S)"
        mkdir -p "$backup_dir"
...
start_service() {
    ...
    if command_exists pm2 && [ "$1" != "--no-pm2" ]; then
        ...
        # 直接使用pm2启动，避免循环调用
        pm2 start "$APP_DIR/src/app.js" --name "ai-relay" --log "$APP_DIR/logs/pm2.log" 2>/dev/null
        sleep 2
        
        # 检查是否启动成功
        if pm2 list 2>/dev/null | grep -q "ai-relay"; then
...
stop_service() {
    ...
    if command_exists pm2 && [ -n "$APP_DIR" ] && [ -d "$APP_DIR" ]; then
        ...
        pm2 stop ai-relay 2>/dev/null || true
        pm2 delete ai-relay 2>/dev/null || true
    fi
...
show_menu() {
    clear
    echo -e "${BOLD}======================================${NC}"
echo -e "${BOLD}  AI Relay Service (CRS) 管理工具  ${NC}"
    echo -e "${BOLD}======================================${NC}"
    echo ""
