...
// Be explicit to load the directory index to avoid resolution ambiguity
...
class Application {
  ...
  async initialize() {
    try {
      ...
      // Dev-friendly prefix: allow /webapi/admin/* to hit the same admin routes
      ...
      this.app.get('/health', async (req, res) => {
        try {
          ...
          // 检查各个组件健康状态
          const [redisHealth, pgHealth, loggerHealth] = await Promise.all([
  ...
  async checkPostgresHealth() {
    try {
      ...
      const latency = Date.now() - start
      return { status: 'healthy', connected: true, latency: `${latency}ms` }
    } catch (error) {
  ...
  async start() {
    try {
      ...
      this.server = this.app.listen(config.server.port, config.server.host, () => {
        logger.start(
          `🚀 AI Relay Service started on ${config.server.host}:${config.server.port}`
        )
        logger.info(
