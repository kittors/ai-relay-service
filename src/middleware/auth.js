...
const authenticateApiKey = async (req, res, next) => {
  ...
  try {
    ...
    // 检查每日请求次数限制（独立于时间窗口限流）
    // 读取每日请求上限；若验证返回未包含该字段，回退直读Redis中的原始key数据
    let dailyRequestsLimit
    const hasDailyLimitField =
      validation.keyData &&
      Object.prototype.hasOwnProperty.call(validation.keyData, 'dailyRequestsLimit')
    if (hasDailyLimitField) {
      dailyRequestsLimit = parseInt(validation.keyData.dailyRequestsLimit || 0)
    } else {
      try {
        const rawKey = await redis.getApiKey(validation.keyData.id)
        dailyRequestsLimit = parseInt(rawKey?.dailyRequestsLimit || '0') || 0
        logger.debug(
          `ℹ️ Loaded dailyRequestsLimit from Redis for key ${validation.keyData.id}: ${dailyRequestsLimit}`
        )
      } catch (e) {
        logger.warn(
          `⚠️ Failed to load dailyRequestsLimit from Redis for key ${validation.keyData.id}: ${e.message}`
        )
    ...
    if (dailyRequestsLimit > 0) {
      ...
      const dailyReqKey = `rate_limit:daily_requests:${validation.keyData.id}:${todayStr}`

      // 先自增再判断，保证原子性；超过则回退并拒绝
      const current = await client.incr(dailyReqKey)
      ...
      if (current === 1) {
        ...
        const ttlMs = Math.max(1000, nextMidnightShifted - tzNow.getTime())
        await client.pexpire(dailyReqKey, ttlMs)
      }

      if (current > dailyRequestsLimit) {
        // 回退计数，避免超额占用额度
        await client.decr(dailyReqKey)

        // 计算重置时间（按配置时区的次日 00:00）
        const tzNow2 = redis.getDateInTimezone(new Date())
        const nextMidnightShifted2 = Date.UTC(
          tzNow2.getUTCFullYear(),
          tzNow2.getUTCMonth(),
          tzNow2.getUTCDate() + 1,
          0,
          0,
        ...
        const resetAtIso = new Date(
          Date.now() + (nextMidnightShifted2 - tzNow2.getTime())
        ).toISOString()

        logger.security(
          `🚦 Daily requests limit exceeded for key: ${validation.keyData.id} (${validation.keyData.name}), requests: ${current - 1}/${dailyRequestsLimit}`
        )

        return res.status(429).json({
          error: 'Daily requests limit exceeded',
          message: `已达到每日请求次数限制 (${dailyRequestsLimit} 次)`,
          currentRequests: current - 1,
          requestLimit: dailyRequestsLimit,
          resetAt: resetAtIso
        })
      }
    }
    ...
    req.apiKey = {
      ...
      dailyRequestsLimit: hasDailyLimitField
        ? parseInt(validation.keyData.dailyRequestsLimit || 0)
        : undefined,
