...
// 虎皮椒支付异步通知
router.post('/xunhu', async (req, res) => {
  try {
    const body = req.body || {}
    const xh = appConfig.payment?.xunhu || {}
    if (!xh.appId || !xh.appSecret) {
      return res.status(400).json({ error: 'XunhuPay not configured' })
    }

    // 验证签名
    const calc = xhSdk.getHash(body, xh.appSecret)
    const provided = body.hash
    if (!provided || calc !== provided) {
      logger.warn('⚠️ Xunhu webhook invalid signature')
      return res.status(400).json({ error: 'invalid sign' })
    }

    const { status } = body // OD=支付成功, WP=待支付, CD=已取消
    const outId = body.out_trade_order
    const openId = body.open_order_id

    if (!outId) {
      return res.status(400).json({ error: 'missing out_trade_order' })
    }

    // 更新订单状态
    await orderService.updateOrder(outId, {
      status,
      openOrderId: openId || ''
    })

    // 如果已支付，尝试按套餐创建 API Key
    if (status === 'OD') {
      const order = await orderService.getOrder(outId)
      if (order && order.planId && !order.apiKeyId) {
        try {
          ...
          logger.error('❌ Failed to create API key for paid order:', e)
        }
      }
    }
    ...
    logger.error('❌ Xunhu webhook error:', error)
