...
const OAUTH_CLIENT_ID = geminiConfig.oauthClientId || ''
const OAUTH_CLIENT_SECRET = geminiConfig.oauthClientSecret || ''
const OAUTH_SCOPES = geminiConfig.oauthScopes || ['https://www.googleapis.com/auth/cloud-platform']
const DEFAULT_REDIRECT_URI = geminiConfig.defaultRedirectUri || 'http://localhost:45462'
const DEFAULT_AUTH_REDIRECT_URI = geminiConfig.defaultAuthRedirectUri ||
  'https://codeassist.google.com/authcode'

if (!OAUTH_CLIENT_ID || !OAUTH_CLIENT_SECRET) {
  logger.warn(
    'Gemini OAuth credentials are not configured. Set GEMINI_OAUTH_CLIENT_ID and GEMINI_OAUTH_CLIENT_SECRET.'
  )
}
...
async function generateAuthUrl(state = null, redirectUri = null, proxyConfig = null) {
  // 使用新的 redirect URI
  const finalRedirectUri = redirectUri || DEFAULT_AUTH_REDIRECT_URI
