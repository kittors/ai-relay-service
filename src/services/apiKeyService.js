...
class ApiKeyService {
  constructor() {
    ...
    // 加密配置（用于可选保存可复制的明文 Token）
    this._ALGORITHM = 'aes-256-cbc'
    this._IV_LENGTH = 16
    this._ENCRYPTION_SALT = 'api-key-plaintext-salt'
  ...
  async generateApiKey(options = {}) {
    const {
      ...
      dailyRequestsLimit = 0, // 新增：每日请求次数上限
    ...
    const keyData = {
      ...
      dailyRequestsLimit: String(dailyRequestsLimit || 0),
    ...
    // 同步持久化到 Postgres（幂等）
    try {
      await apiKeyStore.upsertFromRedisLike(keyData)
    } catch (e) {
      logger.warn(`⚠️ PG upsert for API key ${keyId} failed: ${e.message}`)
    }

    // 额外：安全地保存一次明文 Token（加密存储，供管理员复制，不影响旧 Token 的有效性）
    try {
      await this._storePlaintextToken(keyId, apiKey)
    } catch (e) {
      logger.warn(`⚠️ Failed to store plaintext token for ${keyId}:`, e.message)
    }
    ...
    return {
      ...
      dailyRequestsLimit: parseInt(keyData.dailyRequestsLimit || 0),
  ...
  async validateApiKey(apiKey) {
    try {
      ...
      // 优先从 Postgres 通过 digest 查找；失败则回退 Redis 反向映射
      let keyData = await apiKeyStore.findByDigest(hashedKey)
      if (!keyData) {
      ...
      if (
        ...
        if (activationUnit === 'hours') {
          const ms = activationPeriod * 60 * 60 * 1000
          expiresAt = new Date(now.getTime() + ms)
        } else if (activationUnit === 'months') {
          const m = parseInt(activationPeriod || 1)
          const dt = new Date(now)
          dt.setMonth(dt.getMonth() + m)
          expiresAt = dt
        } else {
          const ms = activationPeriod * 24 * 60 * 60 * 1000
          expiresAt = new Date(now.getTime() + ms)
        }
        ...
        try {
          ...
          logger.warn(`⚠️ PG upsert after activation failed for ${keyData.id}: ${e.message}`)
        }
  ...
  // 🔎 分页/游标列出 API Keys（轻量，不附带使用统计）
  async listApiKeysCursor(options = {}) {
    const {
      cursor = '0',
      count = 20,
    ...
    // Feature flag: 优先使用 PG 列表（更稳定可筛选），否则回退 Redis SCAN
    if (process.env.PG_LIST_API_KEYS === 'true') {
      const res = await apiKeyStore.listCursor({
        cursor,
        count,
        includeDeleted,
        search,
      ...
      const items = (res.items || []).map((data) => {
        ...
        return {
          ...
          isDeleted: data.isDeleted === 'true' || data.isDeleted === true,
          permissions: data.permissions || 'all',
          claudeAccountId: data.claudeAccountId,
          claudeConsoleAccountId: data.claudeConsoleAccountId,
          geminiAccountId: data.geminiAccountId,
          openaiAccountId: data.openaiAccountId,
          azureOpenaiAccountId: data.azureOpenaiAccountId,
          bedrockAccountId: data.bedrockAccountId,
          droidAccountId: data.droidAccountId,
          userId: data.userId,
          userUsername: data.userUsername,
          ownerDisplayName: data.ownerDisplayName,
          tags,
          dailyRequestsLimit: parseInt(data.dailyRequestsLimit || '0') || 0,
          rateLimitWindow: parseInt(data.rateLimitWindow || '0') || 0,
          rateLimitRequests: parseInt(data.rateLimitRequests || '0') || 0,
          expirationMode: data.expirationMode || 'fixed',
          isActivated: data.isActivated === 'true' || data.isActivated === true,
          activationDays: parseInt(data.activationDays || '0') || 0,
          activationUnit: data.activationUnit || 'days'
        }
      })
      return { cursor: res.cursor, finished: res.finished, items }
    }
    ...
    const SCAN_BATCH = Math.max(10, Math.min(1000, Number(count) * 10))
    while (items.length < count) {
      const scanArgs = ['MATCH', 'apikey:*', 'COUNT', String(SCAN_BATCH)]
      const [newCursor, keys] = await client.scan(nextCursor, ...scanArgs)
      nextCursor = newCursor
      const filteredKeys = keys.filter(
        (k) => k !== 'apikey:hash_map' && !k.startsWith('apikey:plaintext:')
      )
      if (filteredKeys.length > 0) {
        const pipeline = client.pipeline()
        filteredKeys.forEach((k) => pipeline.hgetall(k))
        const results = await pipeline.exec()
        for (let i = 0; i < filteredKeys.length; i++) {
          const keyName = filteredKeys[i]
          ...
          if (wantTag && (!Array.isArray(tags) || !tags.includes(wantTag))) {
            continue
          }
          if (normalizedSearch) {
            const hay =
              `${data.name || ''} ${data.ownerDisplayName || ''} ${data.userUsername || ''}`.toLowerCase()
            if (!hay.includes(normalizedSearch)) {
              continue
            }
          }
          if (activated === 'true' && data.isActivated !== 'true') {
            continue
          }
          if (activated === 'false' && data.isActivated === 'true') {
            continue
          }
          items.push({
            id: keyId,
    ...
    return { cursor: nextCursor, finished: nextCursor === '0', items }
  }
  ...
  async updateApiKey(keyId, updates) {
    try {
      ...
      try {
        await apiKeyStore.upsertFromRedisLike(updatedData)
      } catch (e) {
        logger.warn(`⚠️ PG upsert on update failed for ${keyId}: ${e.message}`)
      }
  ...
  async restoreApiKey(keyId, restoredBy = 'system', restoredByType = 'system') {
    try {
      ...
      try {
        await apiKeyStore.restore(keyId, restoredBy, restoredByType)
      } catch (e) {
        logger.warn(`⚠️ PG restore failed for ${keyId}: ${e.message}`)
      }
  ...
  async permanentDeleteApiKey(keyId) {
    try {
      ...
      try {
        await apiKeyStore.permanentDelete(keyId)
      } catch (e) {
        logger.warn(`⚠️ PG permanent delete failed for ${keyId}: ${e.message}`)
      }
  ...
  async getApiKeyById(keyId, userId = null) {
    try {
      let keyData = await apiKeyStore.getById(keyId)
      if (!keyData || Object.keys(keyData).length === 0) {
  ...
  async regenerateApiKey(keyId) {
    try {
      ...
      try {
        await apiKeyStore.upsertFromRedisLike(updatedKeyData)
      } catch (e) {
        logger.warn(`⚠️ PG upsert after regenerate failed for ${keyId}: ${e.message}`)
      }

      // 保存新的明文 Token
      try {
        await this._storePlaintextToken(keyId, newApiKey)
      } catch (e) {
        logger.warn(`⚠️ Failed to store new plaintext token for ${keyId}:`, e.message)
      }
  ...
  // === 明文 Token 加密存取（仅管理员可通过接口读取） ===
  _getDerivedKey() {
    if (!this._derivedKey) {
      this._derivedKey = crypto.scryptSync(config.security.encryptionKey, this._ENCRYPTION_SALT, 32)
    }
  ...
  async _storePlaintextToken(keyId, token) {
    const client = redis.getClientSafe()
    const enc = this._encrypt(token)
    await client.set(`apikey:plaintext:${keyId}`, enc)
  }

  async getPlaintextToken(keyId) {
    try {
      const client = redis.getClientSafe()
      const enc = await client.get(`apikey:plaintext:${keyId}`)
      if (!enc) {
        return null
      }
      const plain = this._decrypt(enc)
      return plain || null
    } catch (e) {
      logger.error('❌ Failed to get plaintext token:', e)
      return null
    }
  }
